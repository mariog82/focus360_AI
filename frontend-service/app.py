...
# ---- Frontend-service integration with API Gateway ----
@app.context_processor
def inject_gateway_config():
    return {
        'GATEWAY_URL': os.getenv('GATEWAY_URL', 'http://gateway:8080'),
        'FRONTEND_MODE': os.getenv('FRONTEND_MODE', 'frontend-service')
    }
...
def system_status():
    ...
    endpoints = {
        ...
        'microservices': f'{gateway_url}/health/services'
    }
    for name, url in endpoints.items():
        try:
            r = requests.get(url, timeout=5)
            checks[name] = {'url': url, 'status_code': r.status_code, 'ok': r.status_code == 200, 'body': r.text[:1500]}
        except Exception as exc:
            checks[name] = {'url': url, 'ok': False, 'error': str(exc)}
    return render_template('system_status.html', checks=checks, gateway_url=gateway_url)
